(function(field, factory, context) {
    ...
    var SlsRoute = function(options) {
        if (options.constructor === Object) {
             ...
             */
            // this.config.startParam = options.startParam != undefined && options.startParam.constructor === String ? options.startParam : '?';
             ...
             */
            // this.config.param = options.param != undefined && options.param.constructor === String ? options.param : '&';


            /**
             * key,value分隔符
             * @property {String} keyValueSplit
             */
            // this.config.keyValueSplit = options.keyValueSplit != undefined && options.keyValueSplit.constructor === String ? options.keyValueSplit : '=';
    ...
    SlsRoute.prototype = {
        ...
        /**
         * 拦截a标签的默认行为，通过绑定事件执行hash，使用此功能可以在匹配到当前a标签的时候，加激活class
         */
        bindRoute: function() {
            var self = this;
            var allA = document.querySelectorAll('a[href^="' + this.config.hash + '"]');
            var urls = getUrlAndHash.call(this);
            for (var i = 0; i < allA.length; i++) {
                allA[i].onclick = (function(n) {
                    return function(e) {
                        var event = e || event;
                        event.preventDefault && event.preventDefault();
                        event.returnValue = false;

                        if (!hasClass(this, self.config.activeClass)) {
